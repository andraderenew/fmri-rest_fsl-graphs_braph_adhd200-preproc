...
def draw_panel(ax, img, letter, title, letter_x=-0.018, letter_y=1.015):
    ax.imshow(img)
    ax.axis("off")
...
def main():
    ...
    # Crop only presentation-only title/caption whitespace.
    # Preserve the full scientific labels and colorbars at the left/right edges.
    matrix = crop_fraction(matrix, left=0.000, right=0.990, top=0.055, bottom=0.915)
    hub = crop_fraction(hub, left=0.020, right=1.000, top=0.070, bottom=0.985)
    fp = crop_fraction(fp, left=0.020, right=1.000, top=0.070, bottom=0.985)
    atlas = crop_fraction(atlas, left=0.020, right=0.995, top=0.070, bottom=0.995)

    fig = plt.figure(figsize=(15.6, 10.8), facecolor="white")
    outer = fig.add_gridspec(
        1, 2,
        width_ratios=[1.08, 1.0],
        left=0.020, right=0.995, top=0.965, bottom=0.030,
        wspace=0.022,
    )
    ...
    png = out / "figure_main_connectome_surface_matrix_v3.png"
    pdf = out / "figure_main_connectome_surface_matrix_v3.pdf"
    ...
    print("MAIN_PUBLICATION_FIGURE_V3_PASSED")
