...
def main() -> None:
    ...
    # BrainSpace's packaged Schaefer-100 parcellation is retained only
    # as an informational reference comparison. The official CBIG fsLR32k
    # CIFTI validated against the public 100x100 matrix is the authoritative
    # vertex-wise labeling used for every surface map.
    print(
        "BRAINSPACE_REFERENCE_COMPARISON "
        f"LH_EXACT={exact_left} LH_MISMATCHES={mismatches_left} "
        f"RH_EXACT={exact_right} RH_MISMATCHES={mismatches_right}"
    )

    present = sorted(
        (
            set(np.unique(cifti_lh).astype(int))
            | set(np.unique(cifti_rh).astype(int))
        )
        - {0}
    )
    ...
    if present != list(range(1, 101)):
        raise RuntimeError(
            "Official CIFTI does not contain exactly parcel keys 1..100"
        )

    lh_nonzero = np.unique(cifti_lh[cifti_lh > 0]).astype(int)
    rh_nonzero = np.unique(cifti_rh[cifti_rh > 0]).astype(int)

    if not np.array_equal(lh_nonzero, np.arange(1, 51)):
        raise RuntimeError(
            f"Unexpected official CIFTI LH keys: {lh_nonzero.tolist()}"
        )

    if not np.array_equal(rh_nonzero, np.arange(51, 101)):
        raise RuntimeError(
            f"Unexpected official CIFTI RH keys: {rh_nonzero.tolist()}"
        )

    print("OFFICIAL_CIFTI_PARCELS_1_TO_100=YES")
    print("OFFICIAL_CIFTI_LH_KEYS_1_TO_50=YES")
    print("OFFICIAL_CIFTI_RH_KEYS_51_TO_100=YES")
    print("SURFACE_MAPPING_SOURCE=OFFICIAL_CBIG_CIFTI_FSLR32K")
